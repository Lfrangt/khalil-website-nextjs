...
export default function NomadMap() {
  ...
  const getCurrentLocation = () => {
    ...
    if (!navigator.geolocation) {
      setLocationError(language === 'zh' ? '您的浏览器不支持定位' : 'Geolocation is not supported');
      setIsLocating(false);
      // Fallback to Vancouver
      const fallbackCoords: [number, number] = [-123.1207, 49.2827];
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        setLocationError(language === 'zh' ? '无法获取位置信息' : 'Unable to get location');
        setIsLocating(false);
        // Fallback to Vancouver
        const fallbackCoords: [number, number] = [-123.1207, 49.2827];
        setCurrentLocation(fallbackCoords);
  ...
  useEffect(() => {
    ...
    try {
      ...
      label.textContent = language === 'zh' ? '你 现在' : 'You Now';
    ...
    <div className="relative w-full h-full">
      ...
      {isSharing && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-black/80 backdrop-blur-md text-white px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 shadow-lg">
          ...
          {language === 'zh' ? '正在共享位置' : 'Sharing Location'}
        </div>
      )}

      {/* Right top - Current location button */}
      <button
        onClick={getCurrentLocation}
        disabled={isLocating}
        className="absolute top-4 right-4 z-20 bg-white/95 backdrop-blur-md p-3 rounded-full shadow-lg hover:bg-white transition-all disabled:opacity-50"
        title={language === 'zh' ? '当前位置' : 'Current Location'}
      >
        {isLocating ? (
